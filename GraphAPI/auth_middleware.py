...
def _get_secret() -> str:
    ...
    print(f"[GraphAPI Auth] Using secret starting with: {secret[:5]}...")
    return secret
...
def jwt_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if not auth_header.startswith("Bearer "):
            print(f"[GraphAPI Auth] Rejecting: Missing or invalid Authorization header. Received: '{auth_header}'")
        ...
        try:
            ...
            print("[GraphAPI Auth] Rejecting: Token is expired")
            return jsonify({"error": "token has expired"}), 401
        except jwt.InvalidTokenError as e:
            print(f"[GraphAPI Auth] Rejecting: Invalid token: {e}")
